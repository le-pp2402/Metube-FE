...
export default function LivePage({ idLiveSession }: { idLiveSession: string }) {
    const id = Number(idLiveSession);
    ...
    useEffect(() => {
        async function fetchLiveSession() {
            try {
                ...
                const sessionData = res?.data?.data || null;
    ...
    const streamUrl = `${NEXT_PUBLIC_LIVE_API_URL}/${liveSession.username}.m3u8`;
